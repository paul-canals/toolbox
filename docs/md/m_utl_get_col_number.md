...
# 
# File Reference: m_utl_get_col_number.sas

### Utilities

##### Utility macro to obtain column number attribute from a table.

***

### Description
The macro is used to obtain the attribute number information from a given column in a table. The macro can be used to get the column number from within a data step, or used standalone. The result information is directly given back from the macro.
...
### Returns
* The order number value for a given column.
...
```sas
%m_utl_get_col_number(?)
```

##### Example 2: Obtain number information for Name column of the SASHELP.class table:
```sas
data WORK.result;
   Table="SASHELP.class";
   Column='Name';
   Number="%m_utl_get_col_number(table=SASHELP.class,col=Name,debug=Y)";
   output;
run;
...
##### Example 3: Obtain number information for Prodtype column of the SASHELP.prdsale table:
```sas
data WORK.result;
   Table="SASHELP.prdsale";
   Column='Prodtype';
   Number="%m_utl_get_col_number(table=SASHELP.prdsale,col=Prodtype,debug=Y)";
   output;
run;
...
##### Example 4: Obtain number information for Invioce column of the SASHELP.cars table:
```sas
data WORK.result;
   Table="SASHELP.cars";
   Column='Invoice';
   Number="%m_utl_get_col_number(table=SASHELP.cars,col=Invoice,debug=Y)";
   output;
run;
